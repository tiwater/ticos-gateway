...
class SDKTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.gateway = TicosGatewayService('data/gateway/gateway.yaml')

    @classmethod
    def tearDownClass(cls) -> None:
        cls.gateway._TicosGatewayService__stop_gateway()

    @patch('ticos_gateway_mqtt.TicosGatewayMqttClient.gw_connect_device')
    ...
    @patch('ticos_gateway_mqtt.TicosGatewayMqttClient.gw_disconnect_device')
    ...
    @patch('ticos_gateway_mqtt.TicosGatewayMqttClient.gw_send_attributes')
    def test_send_attributes(self, mock_func):
        data = {self.DEVICE_NAME: {'attributes': {"atr1": 1, "atr2": True, "atr3": "value3"}}}

        self.gateway.add_device(self.DEVICE_NAME,
                                {'connector': self.gateway.available_connectors['MQTT Broker Connector']}, self.DEVICE_TYPE)
        self.gateway._TicosGatewayService__send_data(data)

        mock_func.assert_called_with(self.DEVICE_NAME, {"atr1": 1, "atr2": True, "atr3": "value3"})

    @patch('ticos_gateway_mqtt.TicosGatewayMqttClient.gw_send_telemetry')
    ...
    @patch('ticos_gateway_mqtt.TicosGatewayMqttClient.gw_subscribe_to_all_attributes')
    ...
    @patch('ticos_device_mqtt.TicosDeviceMqttClient.subscribe_to_all_attributes')
    def test_subscribe_to_all_device_attributes(self, mock_func):
        self.gateway.subscribe_to_required_topics()

        mock_func.assert_called_with(self.gateway._attribute_update_callback)

    @patch('ticos_device_mqtt.TicosDeviceMqttClient.set_server_side_rpc_request_handler')
    ...
    @patch('ticos_gateway_mqtt.TicosGatewayMqttClient.gw_set_server_side_rpc_request_handler')
    def test_gw_set_server_side_rpc_request(self, mock_func):
        self.gateway.subscribe_to_required_topics()

        mock_func.assert_called_with(self.gateway._rpc_request_handler)

    @patch('ticos_gateway_mqtt.TicosGatewayMqttClient.gw_send_rpc_reply')
    def test_gw_send_rpc_reply_with_content(self, mock_func):
        self.gateway._TicosGatewayService__send_rpc_reply(self.DEVICE_NAME, 1,
    ...
    def test_gw_send_rpc_reply_without_content(self, mock_func):
        self.gateway._TicosGatewayService__send_rpc_reply(self.DEVICE_NAME, 1, success_sent=True)

        mock_func.assert_called_with(self.DEVICE_NAME, 1, dumps({'success': True}), quality_of_service=0)

    @patch('ticos_device_mqtt.TicosDeviceMqttClient.send_rpc_reply')
    def test_send_device_rpc_reply_without_content(self, mock_func):
        self.gateway._TicosGatewayService__send_rpc_reply(req_id=1, success_sent=True)
    ...
    def test_send_device_rpc_reply_with_content(self, mock_func):
        self.gateway._TicosGatewayService__send_rpc_reply(req_id=1, content={'device': self.DEVICE_NAME,
                                                                          'data': {'id': 1, 'method': 'echo'}},
                                                       success_sent=True)

        mock_func.assert_called_with(1, dumps({'success': True}), quality_of_service=0, wait_for_publish=None)

    @patch('ticos_gateway_mqtt.TicosGatewayMqttClient.gw_request_shared_attributes')
    @patch('ticos_gateway_mqtt.TicosGatewayMqttClient.gw_request_client_attributes')
