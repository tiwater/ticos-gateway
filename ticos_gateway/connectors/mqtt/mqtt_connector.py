...
from ticos_gateway.gateway.constants import SEND_ON_CHANGE_PARAMETER, DEFAULT_SEND_ON_CHANGE_VALUE, \
...
class MqttConnector(Connector, Thread):
    def __init__(self, gateway, config, connector_type):
        ...
        # RPCs, i.e., remote procedure calls (Ticos towards devices) handlers
        self.load_handlers('serverSideRpc', mandatory_keys['serverSideRpc'], self.__server_side_rpc)

        # Connect requests, i.e., telling Ticos that a device is online even if it does not post telemetry
        self.load_handlers('connectRequests', mandatory_keys['connectRequests'], self.__connect_requests)

        # Disconnect requests, i.e., telling Ticos that a device is offline even if keep-alive has not expired yet
        self.load_handlers('disconnectRequests', mandatory_keys['disconnectRequests'], self.__disconnect_requests)

        # Shared attributes direct requests, i.e., asking Ticos for some shared attribute value
        self.load_handlers('attributeRequests', mandatory_keys['attributeRequests'], self.__attribute_requests)

        # Attributes updates requests, i.e., asking Ticos to send updates about an attribute
        ...
        while not self.__stopped:
            if not self._on_message_queue.empty():
                ...
                if topic_handlers:
                    ...
                    # may produce more than one message towards Ticos. This also means that I cannot return after
                ...
                if topic_handlers:
                    for topic in topic_handlers:
                        ...
                        # Note: device must be added even if it is already known locally: else Ticos
                    ...
                    try:
                        for topic in topic_handlers:
                            ...
                            self.__log.info("Will retrieve attribute %s of %s", found_attribute_names,
                                            found_device_name)
                            self.__gateway.ticos_client.client.gw_request_shared_attributes(
                                found_device_name,
                ...
                if self.__gateway.is_rpc_in_progress(message.topic):
                    log.info("RPC response arrived. Forwarding it to ticos.")
    ...
    def __process_rpc_request(self, content, rpc_config):
        ...
        try:
            ...
            if not expects_response or not defines_timeout:
                self.__log.info("One-way RPC: sending ack to Ticos immediately")
