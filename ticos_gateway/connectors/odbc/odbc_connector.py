...
class OdbcConnector(Connector, Thread):
    ...
    def __check_and_send(self, device_name, device_type, new_data):
        ...
        for ticos_key in to_send.keys():
            for key, new_value in new_data[ticos_key].items():
                if not send_on_change or self.__devices[device_name][ticos_key].get(key, None) != new_value:
                    self.__devices[device_name][ticos_key][key] = new_value
    ...
    def __resolve_iterator_file(self):
        ...
        try:
            ...
            # https://ticos.io/docs/iot-gateway/config/odbc/#subsection-iterator
