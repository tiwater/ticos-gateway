            ...
            for information in device_info["configuration"][information_type]:
                ...
                for information_node in information_nodes:
                    ...
                    if information_node is not None:
                        ...
                        log.debug("Data to Ticos: %s", converted_data)
                    ...
                    for attribute_node in attribute_nodes:
                        if attribute_node is not None:
                            if self.get_node_path(attribute_node) ==  attribute_path:
                                self.__available_object_resources[device_name]["variables"].append({attribute_update["attributeOnTicos"]: attribute_node})
                        else:
...
class SubHandler(object):
    ...
    def datachange_notification(self, node, val, data):
        try:
            ...
            log.debug("[SUBSCRIPTION] Data to Ticos: %s", converted_data)
        except KeyError:
