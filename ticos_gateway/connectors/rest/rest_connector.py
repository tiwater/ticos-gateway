...
class RESTConnector(Connector, Thread):
    ...
    def load_endpoints(self):
        ...
        if len(self.__config.get('attributeRequests', [])):
            self.__attribute_type = {
                'client': self.__gateway.ticos_client.client.gw_request_client_attributes,
                'shared': self.__gateway.ticos_client.client.gw_request_shared_attributes
            }
        ...
        if self.__config.get('SSL', False):
            if not self.__config.get('security'):
                if not os.path.exists('domain_srv.crt'):
                    from ticos_gateway.connectors.rest.ssl_generator import SSLGenerator
    ...
    def __fill_requests_from_TB(self):
        ...
        for request_section in requests_from_ticos:
            for request_config_object in self.__config.get(request_section, []):
                ...
                requests_from_ticos[request_section].append(request_dict)
