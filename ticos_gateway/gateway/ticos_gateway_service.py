...
from ticos_gateway.gateway.constant_enums import DeviceActions, Status
from ticos_gateway.gateway.constants import CONNECTED_DEVICES_FILENAME, CONNECTOR_PARAMETER, \
    PERSISTENT_GRPC_CONNECTORS_KEY_FILENAME
from ticos_gateway.gateway.device_filter import DeviceFilter
...
class TBGatewayService:
    ...
    def __init__(self, config_file=None):
        ...
        if config_file is None:
            config_file = path.dirname(path.dirname(path.abspath(__file__))) + '/config/ticos_gateway.yaml'.replace('/',
                                                                                                                 path.sep)
        ...
        log.info("Ticos IoT gateway version: %s", self.version["current_version"])
        ...
        if self.__config["ticos"].get("remoteShell"):
        ...
        self.__devices_idle_checker = self.__config['ticos'].get('checkingDeviceActivity', {})
        ...
        self.__statistics = self.__config['ticos'].get('statistics', DEFAULT_STATISTIC)
        ...
        self.__min_pack_send_delay_ms = self.__config['ticos'].get('minPackSendDelayMS', 200)
        ...
        self.__min_pack_size_to_send = self.__config['ticos'].get('minPackSizeToSend', 50)

        self._send_thread = Thread(target=self.__read_data_from_storage, daemon=True,
                                   name="Send data to Ticos Thread")
        self._send_thread.start()

        self.__device_filter_config = self.__config['ticos'].get('deviceFiltering', DEFAULT_DEVICE_FILTER)
    ...
    def _watchers(self):
        try:
            ...
            while not self.stopped:
                ...
                if not self.ticos_client.is_connected() and self.__subscribed_to_rpc_topics:
                    self.__subscribed_to_rpc_topics = False

                if self.ticos_client.is_connected() and not self.__subscribed_to_rpc_topics:
                ...
                if (
                        self.__rpc_requests_in_progress or not self.__rpc_register_queue.empty()) and self.ticos_client.is_connected():
                ...
                if not self.__request_config_after_connect and self.ticos_client.is_connected() and not self.ticos_client.client.get_subscriptions_in_progress():
                    self.__request_config_after_connect = True
                    self.__check_shared_attributes()

                if cur_time - gateway_statistic_send > self.__statistics[
                        'statsSendPeriodInSeconds'] * 1000 and self.ticos_client.is_connected():
                    summary_messages = self.__form_statistics()
                    # with self.__lock:
                    self.ticos_client.client.send_telemetry(summary_messages)
                    gateway_statistic_send = time() * 1000
                    # self.__check_shared_attributes()

                if cur_time - connectors_configuration_check_time > self.__config["ticos"].get(
            ...
            self.ticos_client.stop()

    def __modify_main_config(self):
        env_variables = get_env_variables()
        self.__config['ticos'] = {**self.__config['ticos'], **env_variables}
    ...
    def __init_remote_configuration(self, force=False):
        if (self.__config["ticos"].get("remoteConfiguration") or force) and self.__remote_configurator is None:
            try:
                self.__remote_configurator = RemoteConfigurator(self, self.__config)
                if self.ticos_client.is_connected() and not self.ticos_client.client.get_subscriptions_in_progress():
                    self.__check_shared_attributes()
            except Exception as e:
                log.exception(e)
    ...
    def subscribe_to_required_topics(self):
        self.ticos_client.client.clean_device_sub_dict()
        self.ticos_client.client.gw_set_server_side_rpc_request_handler(self._rpc_request_handler)
        self.ticos_client.client.set_server_side_rpc_request_handler(self._rpc_request_handler)
        self.ticos_client.client.subscribe_to_all_attributes(self._attribute_update_callback)
        self.ticos_client.client.gw_subscribe_to_all_attributes(self._attribute_update_callback)

    def request_device_attributes(self, device_name, shared_keys, client_keys, callback):
        if client_keys is not None:
            self.ticos_client.client.gw_request_client_attributes(device_name, client_keys, callback)
        if shared_keys is not None:
            self.ticos_client.client.gw_request_shared_attributes(device_name, shared_keys, callback)

    def __check_shared_attributes(self):
        self.ticos_client.client.request_attributes(callback=self._attributes_parse)
                ...
                if not self.__converted_data_queue.empty():
                    ...
                    for data in data_array:
                        if not connector_name == self.name:
                            ...
                            if data["deviceName"] not in self.get_devices() and self.ticos_client.is_connected():
                            ...
                            data["deviceName"] = "currentTicosGateway"
                        ...
                        max_data_size = self.__config["ticos"].get("maxPayloadSizeBytes", 400)
    ...
    def check_size(self, devices_data_in_event_pack):
        if self.__get_data_size(devices_data_in_event_pack) >= self.__config["ticos"].get("maxPayloadSizeBytes",
                                                                                                400):
    ...
    def __read_data_from_storage(self):
        ...
        while not self.stopped:
            try:
                if self.ticos_client.is_connected():
                    ...
                    if events:
                        ...
                        if devices_data_in_event_pack:
                            if not self.ticos_client.is_connected():
                                continue
                        ...
                        if self.ticos_client.is_connected() and (
                            ...
                            while not self._published_events.empty():
                                if (self.__remote_configurator is not None and self.__remote_configurator.in_process) or \
                                        not self.ticos_client.is_connected() or \
                                ...
                                for event in events:
                                    try:
                                        ...
                                        success = False
                                    sleep(self.__min_pack_send_delay_ms)
                            if success and self.ticos_client.is_connected():
        ...
        try:
            for device in devices_data_in_event_pack:
                ...
                if devices_data_in_event_pack[device].get("attributes"):
                    if device == self.name or device == "currentTicosGateway":
                        self._published_events.put(
                            self.ticos_client.client.send_attributes(devices_data_in_event_pack[device]["attributes"]))
                    else:
                        self._published_events.put(self.ticos_client.client.gw_send_attributes(final_device_name,
                                                                                            devices_data_in_event_pack[
                                                                                                device]["attributes"]))
                if devices_data_in_event_pack[device].get("telemetry"):
                    if device == self.name or device == "currentTicosGateway":
                        self._published_events.put(
                            self.ticos_client.client.send_telemetry(devices_data_in_event_pack[device]["telemetry"]))
                    else:
                        self._published_events.put(self.ticos_client.client.gw_send_telemetry(final_device_name,
                                                                                           devices_data_in_event_pack[
                                                                                               device]["telemetry"]))
    ...
    def __send_rpc_reply(self, device=None, req_id=None, content=None, success_sent=None, wait_for_publish=None,
        ...
        try:
            ...
            if device is not None and success_sent is not None:
                self.ticos_client.client.gw_send_rpc_reply(device, req_id, dumps(rpc_response),
                                                        quality_of_service=quality_of_service)
            elif device is not None and req_id is not None and content is not None:
                self.ticos_client.client.gw_send_rpc_reply(device, req_id, content, quality_of_service=quality_of_service)
            elif device is None and success_sent is not None:
                self.ticos_client.client.send_rpc_reply(req_id, dumps(rpc_response), quality_of_service=quality_of_service,
                                                     wait_for_publish=wait_for_publish)
            elif device is None and content is not None:
                self.ticos_client.client.send_rpc_reply(req_id, content, quality_of_service=quality_of_service,
                                                     wait_for_publish=wait_for_publish)
            self.__rpc_reply_sent = False
        except Exception as e:
            log.exception(e)
    ...
    def add_device(self, device_name, content, device_type=None):
        if device_name not in self.__saved_devices:
            ...
            self.__save_persistent_devices()
            self.ticos_client.client.gw_connect_device(device_name, device_type)
    ...
    def del_device(self, device_name):
        self.ticos_client.client.gw_disconnect_device(device_name)
    ...
    # Gateway ----------------------
    def get_status(self):
        return {'connected': self.ticos_client.is_connected()}
