...
from ticos_gateway.connectors.mqtt.mqtt_connector import MqttConnector, MQTT_VERSIONS, RESULT_CODES_V5, \
    RESULT_CODES_V3
        ...
        while not self.__stopped:
            if not self._on_message_queue.empty():
                ...
                if topic_handlers:
                    ...
                    # This means that a single MQTT message may produce more than one message towards Ticos.
    ...
    def __process_rpc_request(self, content, rpc_config):
        ...
        try:
            ...
            if not expects_response or not defines_timeout:
                log.info("One-way RPC: sending ack to Ticos immediately")
