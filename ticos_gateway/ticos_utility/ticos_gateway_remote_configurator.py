...
class RemoteConfigurator:
    ...
    def process_configuration(self, configuration):
        try:
            if not self.in_process:
                ...
                self.__new_general_configuration_file = self.__new_configuration.get("ticos")
                ...
                if self.__old_configuration.get("ticos").get("ticos").get("remoteShell"):
                    self.__new_configuration["ticos"]["ticos"]["remoteShell"] = True
    ...
    def send_current_configuration(self):
        try:
            ...
            current_configuration["ticos"] = self.__old_general_configuration_file
            current_configuration["ticos"]["logs"] = b64encode(self.__old_logs_configuration.replace('\n', '}}').encode("UTF-8"))
            ...
            self.__gateway.ticos_client.client.send_attributes(
                {"current_configuration": encoded_current_configuration.decode("UTF-8")})
            LOG.debug('Current configuration has been sent to Ticos: %s', json_current_configuration)
        except Exception as e:
            LOG.exception(e)
    ...
    def __process_connectors_configuration(self):
        ...
        if self.__safe_apply_connection_configuration():
            ...
            with open(self.__gateway.get_config_path() + "ticos_gateway.yaml", "w", encoding="UTF-8") as general_configuration_file:
    ...
    def __prepare_connectors_configuration(self, input_connector_config):
        try:
            ...
            for connector in input_connector_config['ticos']['connectors']:
    ...
    def __safe_apply_connection_configuration(self):
        ...
        self.__old_ticos_client = self.__gateway.ticos_client
        try:
            ...
            while time() * 1000 - apply_start < self.__apply_timeout * 1000 and not connection_state:
                connection_state = self.__gateway.ticos_client.is_connected()
                sleep(.1)
            if not connection_state:
                self.__revert_configuration()
                LOG.info("The gateway cannot connect to the Ticos server with a new configuration.")
                return False
            else:
                self.__old_ticos_client.stop()
